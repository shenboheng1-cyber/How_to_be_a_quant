"""生成 5 条曲线数据: 30/70组合, 三因子, 五因子, 纯multi_asset, 沪深300 (2023-2026)。"""
...
TD = 252
RISK = {"vol_60d": -0.15, "max_drawdown_60d": 0.10}
...
def strat_ret(fac, wts, px, uni, market):
    sc = score_factors_with_weights(fac, wts, score_column="s")
    w = make_robust_monthly_weights(sc, px, uni, market_nav=market, cash_code="511880", **TIGHT)
...
def main():
    ...
    r3 = strat_ret(fac, {**F3, **RISK}, px, uni, market)
    r5 = strat_ret(fac, {**F5, **RISK}, px, uni, market)
    ...
    h = market.pct_change()
    c = r3.dropna().index.intersection(ma.dropna().index).intersection(h.dropna().index); c = c[c >= "2023-01-01"]
    r3, r5, ma, h = r3.reindex(c), r5.reindex(c), ma.reindex(c), h.reindex(c)
    blend = 0.3 * r3 + 0.7 * ma
    series = {"30/70组合": blend, "三因子": r3, "五因子": r5, "multi_asset": ma, "沪深300": h}

    def mets(r):
        r = r.dropna(); nav = (1 + r).cumprod(); ann = nav.iloc[-1] ** (TD / len(r)) - 1; vol = r.std() * np.sqrt(TD)
        return dict(ann=round(ann, 4), sharpe=round(ann / vol, 2), mdd=round(float((nav / nav.cummax() - 1).min()), 4), cum=round(nav.iloc[-1] - 1, 4))
    print("METRICS=" + json.dumps({k: mets(v) for k, v in series.items()}, ensure_ascii=False))
    navs = pd.DataFrame({k: (1 + v).cumprod() for k, v in series.items()})
    navs = navs / navs.iloc[0]
    mp = navs.resample("ME").last()
    print("PTS=" + json.dumps([{"d": i.strftime("%Y-%m"), **{k: round(row[k], 3) for k in series}} for i, row in mp.iterrows()], ensure_ascii=False))
